...
// SetSize retargets the encoder at a new coded size (the X screen was
// resized to track the client viewport). Running encoder restarts in place;
// subscribers ride through it — the new run opens with an IDR whose SPS
...
func (s *Streamer) SetSize(w, h int) {
	...
	s.cfg.W, s.cfg.H = w, h
	if s.running {
		log.Printf("stream: resizing encoder to %dx%d", w, h)
		s.stopLocked()
		s.startLocked()
	}
}
...
func (s *Streamer) startLocked() {
	...
	s.gen++
	gen := s.gen
	log.Printf("stream: encoder started pid=%d %dx%d@%dfps %dk preset=%s",
		cmd.Process.Pid, s.cfg.W, s.cfg.H, s.cfg.FPS, s.cfg.BitrateK, s.cfg.Preset)
	if stderr != nil {
		go logStderr(stderr)
	}
