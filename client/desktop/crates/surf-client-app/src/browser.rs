...
#[cfg(test)]
mod tests {
    use super::{BrowserState, reader_text};
    ...
    #[test]
    fn selects_preserve_server_selection_and_anchor() {
        ...
        let prompt = state.select.as_ref().unwrap();
        assert_eq!(prompt.selected, vec![true]);
        assert_eq!(prompt.rect, Some([0.1, 0.2, 0.3, 0.4]));
    }
}
